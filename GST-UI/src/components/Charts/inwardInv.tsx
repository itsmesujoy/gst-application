...
const options: ApexOptions = {
  ...
  labels: [
    'Inward Invoices not posted in ERP',
    'Matched data',
    'Invoices posted in ERP without corresponding inward documents',
  ],
  legend: {
    show: false,
    position: 'bottom',
...
const InwardInv: React.FC = () => {
  ...
  const handleDataPointSelection = (event: any, chartContext: any, { dataPointIndex }: any) => {
    ...
    if (dataPointIndex === 1) {
      ...
      navigate('/InwardInv/matched-data');
    } else if (dataPointIndex === 0) {
      navigate('/InwardInv/unmatched-data');
    }
  };
  ...
  return (
    <div className="sm:px-7.5 col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark xl:col-span-5">
      <div className="mb-3 justify-between gap-4 sm:flex">
        <div>
          <h5 className="text-xl font-semibold text-black dark:text-white">Inward Invoces</h5>
        </div>
      ...
      <div className="-mx-8 flex flex-wrap items-center justify-center gap-y-3">
        <div className="sm:w-1/2 w-full px-8">
          <div className="flex w-full items-center">
            <span className="mr-2 block h-3 w-full max-w-3 rounded-full bg-[#FF9F1C]"></span>
            <p className="flex w-full justify-between text-sm font-medium text-black dark:text-white">
              <span>Inward Invoices not posted in ERP</span>
        ...
        <div className="sm:w-1/2 w-full px-8">
          <div className="flex w-full items-center">
            <span className="mr-2 block h-3 w-full max-w-3 rounded-full bg-[#2EC4B6]"></span>
            <p className="flex w-full justify-between text-sm font-medium text-black dark:text-white">
        ...
        <div className="sm:w-1/2 w-full px-8">
          <div className="flex w-full items-center">
            <span className="mr-2 block h-3 w-full max-w-3 rounded-full bg-[#E71D36]"></span>
            <p className="flex w-full justify-between text-sm font-medium text-black dark:text-white">
              <span>Invoices posted in ERP without corresponding inward documents</span>
